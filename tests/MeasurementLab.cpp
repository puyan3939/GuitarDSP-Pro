...
struct OversamplingResult
{
    ...
    double ampAliasDb = 0.0;
    double pedalAliasDb = 0.0;
    double ampSpectralDeltaTo16Db = 0.0;
    double pedalSpectralDeltaTo16Db = 0.0;
    std::vector<float> ampOneKhz;
    std::vector<float> pedalOneKhz;
};
...
float binMagnitude(const std::vector<float>& mag, double hz)
{
    ...
    float best = 0.0f;
    for (int b = centre - 1; b <= centre + 1; ++b) best = juce::jmax(best, mag[(size_t)b]);
    return best;
}
...
{
    hz = std::fmod(std::abs(hz), sampleRate);
    if (hz > sampleRate * 0.5) hz = sampleRate - hz;
    return hz;
}
...
double aliasDb(const std::vector<float>& signal, double fundamental)
{
    ...
    for (int h = 3; h <= 8; ++h)
    {
        const double original = fundamental * h;
        if (original < sampleRate * 0.5) continue;
        const double alias = foldFrequency(original);
        if (alias < 40.0 || std::abs(alias - fundamental) < 80.0) continue;
        const double m = binMagnitude(mag, alias);
        e += m * m;
    }
    return 20.0 * std::log10(juce::jmax(1.0e-12, std::sqrt(e) / fund));
}
...
{
    const auto ma = spectrum(a), mr = spectrum(reference);
    double sum = 0.0;
    int count = 0;
    for (int bin = 1; bin < (int)ma.size(); ++bin)
    {
        const double hz = sampleRate * bin / fftSize;
        if (hz < 80.0 || hz > 20000.0) continue;
        const double ref = mr[(size_t)bin];
        if (ref < 1.0e-6) continue;
        const double ad = 20.0 * std::log10(juce::jmax(1.0e-12, (double)ma[(size_t)bin]));
        const double rd = 20.0 * std::log10(juce::jmax(1.0e-12, ref));
        sum += std::abs(ad - rd);
        ++count;
    }
...
std::vector<float> renderAmpSine(int order, float hz, float amplitude)
{
    ...
    for (int base = 0; base < fftSize * 2; base += blockSize)
    {
        ...
        {
            block.setSample(0, i, amplitude * (float)std::sin(phase));
            phase += inc;
            if (phase >= juce::MathConstants<double>::twoPi) phase -= juce::MathConstants<double>::twoPi;
        }
        amp.process(block);
        if (base >= fftSize)
            for (int i = 0; i < blockSize; ++i) output.push_back(block.getSample(0, i));
    }
    return output;
}

std::vector<float> renderPedalSine(int order, float hz, float amplitude)
{
...
double pedalCpuPercent(int order)
{
    ...
    pedal.setType(guitardsp::hq::PedalType::hardDistortion);
    guitardsp::hq::PedalParams p;
    p.drive = 0.88f; p.tone = 0.68f; p.lowCutHz = 72.0f; p.focusHz = 1050.0f; p.midDb = 2.0f;
    pedal.setParameters(p);
...
bool latencyEstimatorSmoke()
{
    ...
    for (size_t i = 0; i < probe.size(); ++i) capture[(size_t)expectedStart + i] = probe[i];
    float corr = 0.0f;
    const int detected = guitardsp::LatencyProbe::estimateDelaySamples(probe, capture.data(), (int)capture.size(), corr);
...
void writeJson(const juce::File& file, const std::array<OversamplingResult,5>& results)
{
    ...
    for (size_t i = 0; i < results.size(); ++i)
    {
        ...
        out << "    {\"factor\": " << r.factor
            ...
            << ", \"ampAliasDb\": " << r.ampAliasDb
            << ", \"pedalAliasDb\": " << r.pedalAliasDb
            << ", \"ampSpectralDeltaTo16Db\": " << r.ampSpectralDeltaTo16Db
            << ", \"pedalSpectralDeltaTo16Db\": " << r.pedalSpectralDeltaTo16Db << "}"
            << (i + 1 == results.size() ? "\n" : ",\n");
    }
    out << "  ]\n}\n";
}
...
int main(int argc, char** argv)
{
    ...
    std::array<OversamplingResult,5> results;
    std::array<std::vector<float>,5> ampOneKhz, pedalOneKhz;

    for (int order = 0; order <= 4; ++order)
    {
        auto& r = results[(size_t)order];
        guitardsp::hq::AmpEngineHQ amp(order);
        amp.prepare(sampleRate, blockSize);
        guitardsp::hq::PedalEngineHQ pedal(order);
        pedal.prepare(sampleRate, blockSize);
        ...
        ampOneKhz[(size_t)order] = renderAmpSine(order, 1000.0f, 0.34f);
        pedalOneKhz[(size_t)order] = renderPedalSine(order, 1000.0f, 0.34f);
        const auto ampHigh = renderAmpSine(order, 9000.0f, 0.42f);
        const auto pedalHigh = renderPedalSine(order, 9000.0f, 0.42f);

        r.ampThdDb = thdDb(ampOneKhz[(size_t)order], 1000.0);
        r.pedalThdDb = thdDb(pedalOneKhz[(size_t)order], 1000.0);
        r.ampAliasDb = aliasDb(ampHigh, 9000.0);
        r.pedalAliasDb = aliasDb(pedalHigh, 9000.0);
        ...
        if (r.factor != expectedFactor || pedal.getOversamplingFactor() != expectedFactor
            ...
            || !std::isfinite(r.ampAliasDb) || !std::isfinite(r.pedalAliasDb))
            ok = false;
    }

    for (int order = 0; order <= 4; ++order)
    {
        results[(size_t)order].ampSpectralDeltaTo16Db = spectralDelta(ampOneKhz[(size_t)order], ampOneKhz[4]);
        results[(size_t)order].pedalSpectralDeltaTo16Db = spectralDelta(pedalOneKhz[(size_t)order], pedalOneKhz[4]);
    }

    for (const auto& r : results)
    {
        std::cout << "OS " << r.factor << "x"
                  << " amp[lat=" << r.ampLatencySamples << " cpu=" << r.ampCpuRealtimePercent << "% THD=" << r.ampThdDb
                  << " alias=" << r.ampAliasDb << " delta16=" << r.ampSpectralDeltaTo16Db << "]"
                  << " pedal[lat=" << r.pedalLatencySamples << " cpu=" << r.pedalCpuRealtimePercent << "% THD=" << r.pedalThdDb
                  << " alias=" << r.pedalAliasDb << " delta16=" << r.pedalSpectralDeltaTo16Db << "]\n";
    }
