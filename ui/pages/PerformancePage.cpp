...
PerformancePage::PerformancePage(AudioEngine& e) : engine(e)
{
    ...
    setupTitle(inputTitle,"PICKUP / INPUT LOAD");setupTitle(pitchTitle,"EXPRESSION PITCH / WHAMMY");setupTitle(rigTitle,"MULTI-AMP / LEVEL / ALIGNMENT");setupTitle(delayTitle,"DUAL DELAY");setupTitle(sceneTitle,"SCENE / OUTPUT");

    addAndMakeVisible(inputEnabled);
    setupKnob(pickupR,"PICKUP R",1.5,30.0,6.5,0.1," kOhm");setupKnob(pickupL,"PICKUP L",0.2,12.0,3.2,0.05," H");setupKnob(cableC,"CABLE C",50,3000,470,10," pF");setupKnob(inputZ,"INPUT Z",22,10000,1000,10," kOhm");setupKnob(inputTrim,"TRIM",-12,12,0,0.1," dB");

    addAndMakeVisible(pitchEnabled);for(auto* b:{&pitchMain,&pitchClean,&pitchSub})addAndMakeVisible(*b);
    setupKnob(pitchSemitones,"SEMITONES",-24,24,12,0.1," st");setupKnob(pitchExpression,"EXPR",0,1,0,0.001);setupKnob(pitchWet,"WET",0,1,1,0.001);setupKnob(pitchDry,"DRY",0,1,0,0.001);setupKnob(pitchTracking,"TRACK",0,1,.55,0.001);setupKnob(pitchTone,"TONE",0,1,.75,0.001);setupKnob(pitchSmooth,"SMOOTH",0,1,.60,0.001);

    for(auto* b:{&rigEnabled,&cleanEnabled,&subEnabled,&autoLatency})addAndMakeVisible(*b);
    setupKnob(mainLevel,"MAIN",-60,12,0,.1," dB");setupKnob(cleanLevel,"CLEAN",-60,12,-12,.1," dB");setupKnob(subLevel,"SUB",-60,12,-8,.1," dB");
    setupKnob(cleanBass,"CLEAN BASS",-12,12,0,.1," dB");setupKnob(cleanMid,"CLEAN MID",-12,12,-1,.1," dB");setupKnob(cleanTreble,"CLEAN TREBLE",-12,12,2.5,.1," dB");setupKnob(subBass,"SUB BASS",-12,12,2,.1," dB");setupKnob(subMid,"SUB MID",-12,12,.5,.1," dB");setupKnob(subTreble,"SUB TREBLE",-12,12,-2,.1," dB");

    addAndMakeVisible(dualDelayEnabled);
    setupKnob(delayL,"TIME L",1,2000,280,1," ms");setupKnob(delayR,"TIME R",1,2000,420,1," ms");setupKnob(feedbackL,"FB L",-.95,.95,.28,.001);setupKnob(feedbackR,"FB R",-.95,.95,.32,.001);setupKnob(crossFeedback,"X-FB",-.9,.9,.12,.001);setupKnob(delayMix,"MIX",0,1,.20,.001);setupKnob(delayLowCut,"LOW CUT",20,800,120,1," Hz");setupKnob(delayHighCut,"HIGH CUT",1200,18000,9000,10," Hz");setupKnob(delayModRate,"MOD RATE",.01,8,.35,.01," Hz");setupKnob(delayModDepth,"MOD DEPTH",0,8,.7,.01," ms");
    ...
    rigEnabled.onClick=[this]{pushRigDetail();};cleanEnabled.onClick=[this]{pushRigDetail();};subEnabled.onClick=[this]{pushRigDetail();};autoLatency.onClick=[this]{pushRigDetail();};for(auto* s:{&mainLevel,&cleanLevel,&subLevel,&cleanBass,&cleanMid,&cleanTreble,&subBass,&subMid,&subTreble})s->onValueChange=[this]{pushRigDetail();};
    ...
    outputMode.onChange=[this]{if(!updating)engine.setOutputMode(outputMode.getSelectedId()==2?SignalChain::OutputMode::mainLeftSubRight:SignalChain::OutputMode::stereoMix);};sceneCapture.onClick=[this]{captureScene();};sceneRecall.onClick=[this]{recallScene();};
    refreshFromEngine();
}
...
void PerformancePage::refreshFromEngine()
{
    ...
    auto& r=engine.getHQEffectsRack().parallelRigControl();rigEnabled.setToggleState(r.enabled.load(),juce::dontSendNotification);cleanEnabled.setToggleState(r.cleanEnabled.load(),juce::dontSendNotification);subEnabled.setToggleState(r.subEnabled.load(),juce::dontSendNotification);autoLatency.setToggleState(r.autoLatencyComp.load(),juce::dontSendNotification);mainLevel.setValue(r.mainLevelDb.load(),juce::dontSendNotification);cleanLevel.setValue(r.cleanLevelDb.load(),juce::dontSendNotification);subLevel.setValue(r.subLevelDb.load(),juce::dontSendNotification);cleanBass.setValue(r.cleanBassDb.load(),juce::dontSendNotification);cleanMid.setValue(r.cleanMidDb.load(),juce::dontSendNotification);cleanTreble.setValue(r.cleanTrebleDb.load(),juce::dontSendNotification);subBass.setValue(r.subBassDb.load(),juce::dontSendNotification);subMid.setValue(r.subMidDb.load(),juce::dontSendNotification);subTreble.setValue(r.subTrebleDb.load(),juce::dontSendNotification);
    ...
    outputMode.setSelectedId(engine.getOutputMode()==SignalChain::OutputMode::mainLeftSubRight?2:1,juce::dontSendNotification);updating=false;
}
...
void PerformancePage::pushPitch(){if(updating)return;auto& c=engine.getExpressionPitchControl();int mask=0;if(pitchMain.getToggleState())mask|=HQEffectsRack::routeMain;if(pitchClean.getToggleState())mask|=HQEffectsRack::routeClean;if(pitchSub.getToggleState())mask|=HQEffectsRack::routeSub;if(mask==0)mask=HQEffectsRack::routeMain;c.enabled.store(pitchEnabled.getToggleState());c.routeMask.store(mask);c.semitones.store((float)pitchSemitones.getValue());c.expression.store((float)pitchExpression.getValue());c.wet.store((float)pitchWet.getValue());c.dry.store((float)pitchDry.getValue());c.tracking.store((float)pitchTracking.getValue());c.tone.store((float)pitchTone.getValue());c.smooth.store((float)pitchSmooth.getValue());}
void PerformancePage::pushRigDetail(){if(updating)return;auto& c=engine.getHQEffectsRack().parallelRigControl();c.enabled.store(rigEnabled.getToggleState());c.cleanEnabled.store(cleanEnabled.getToggleState());c.subEnabled.store(subEnabled.getToggleState());c.autoLatencyComp.store(autoLatency.getToggleState());c.mainLevelDb.store((float)mainLevel.getValue());c.cleanLevelDb.store((float)cleanLevel.getValue());c.subLevelDb.store((float)subLevel.getValue());c.cleanBassDb.store((float)cleanBass.getValue());c.cleanMidDb.store((float)cleanMid.getValue());c.cleanTrebleDb.store((float)cleanTreble.getValue());c.subBassDb.store((float)subBass.getValue());c.subMidDb.store((float)subMid.getValue());c.subTrebleDb.store((float)subTreble.getValue());}
...
{g.fillAll(juce::Colour::fromRGB(12,16,21));g.setColour(juce::Colour::fromRGB(28,34,42));g.drawRoundedRectangle(getLocalBounds().toFloat().reduced(10.0f),12.0f,1.0f);g.setColour(juce::Colour::fromRGB(150,158,168));g.setFont(10.5f);for(auto* s:{&pickupR,&pickupL,&cableC,&inputZ,&inputTrim,&pitchSemitones,&pitchExpression,&pitchWet,&pitchDry,&pitchTracking,&pitchTone,&pitchSmooth,&mainLevel,&cleanLevel,&subLevel,&cleanBass,&cleanMid,&cleanTreble,&subBass,&subMid,&subTreble,&delayL,&delayR,&feedbackL,&feedbackR,&crossFeedback,&delayMix,&delayLowCut,&delayHighCut,&delayModRate,&delayModDepth})g.drawText(s->getName(),s->getBounds().withHeight(16).translated(0,-12),juce::Justification::centred);}

void PerformancePage::resized()
{
    auto r=getLocalBounds().reduced(22);title.setBounds(r.removeFromTop(38));
    auto section=[&](juce::Label& l,int h){auto s=r.removeFromTop(h);l.setBounds(s.removeFromTop(22));return s;};
    auto in=section(inputTitle,108);auto ih=in.removeFromTop(24);inputEnabled.setBounds(ih.removeFromLeft(130));int iw=juce::jmax(82,in.getWidth()/5);for(auto* s:{&pickupR,&pickupL,&cableC,&inputZ,&inputTrim})s->setBounds(in.removeFromLeft(iw).reduced(3,8));r.removeFromTop(4);
    auto pi=section(pitchTitle,120);auto ph=pi.removeFromTop(24);pitchEnabled.setBounds(ph.removeFromLeft(110));pitchMain.setBounds(ph.removeFromLeft(75));pitchClean.setBounds(ph.removeFromLeft(80));pitchSub.setBounds(ph.removeFromLeft(70));int pw=juce::jmax(78,pi.getWidth()/7);for(auto* s:{&pitchSemitones,&pitchExpression,&pitchWet,&pitchDry,&pitchTracking,&pitchTone,&pitchSmooth})s->setBounds(pi.removeFromLeft(pw).reduced(3,8));r.removeFromTop(4);
    auto rig=section(rigTitle,132);auto rh=rig.removeFromTop(24);rigEnabled.setBounds(rh.removeFromLeft(110));cleanEnabled.setBounds(rh.removeFromLeft(85));subEnabled.setBounds(rh.removeFromLeft(70));autoLatency.setBounds(rh.removeFromLeft(135));int rw=juce::jmax(74,rig.getWidth()/9);for(auto* s:{&mainLevel,&cleanLevel,&subLevel,&cleanBass,&cleanMid,&cleanTreble,&subBass,&subMid,&subTreble})s->setBounds(rig.removeFromLeft(rw).reduced(2,8));r.removeFromTop(4);
    auto de=section(delayTitle,116);auto dh=de.removeFromTop(24);dualDelayEnabled.setBounds(dh.removeFromLeft(130));int dw=juce::jmax(72,de.getWidth()/10);for(auto* s:{&delayL,&delayR,&feedbackL,&feedbackR,&crossFeedback,&delayMix,&delayLowCut,&delayHighCut,&delayModRate,&delayModDepth})s->setBounds(de.removeFromLeft(dw).reduced(2,8));r.removeFromTop(4);
    auto sc=section(sceneTitle,50);sceneSelector.setBounds(sc.removeFromLeft(120).reduced(3));sceneCapture.setBounds(sc.removeFromLeft(90).reduced(3));sceneRecall.setBounds(sc.removeFromLeft(90).reduced(3));sc.removeFromLeft(20);outputMode.setBounds(sc.removeFromLeft(270).reduced(3));
}
