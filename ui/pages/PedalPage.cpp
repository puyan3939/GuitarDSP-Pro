...
PedalPage::PedalPage(HQEffectsRack& rack) : effectsRack(rack)
{
    ...
    const char* models[] = {"Clean Boost","Treble Boost","Mid Overdrive","Transparent OD","Hard Distortion","Germanium Fuzz","Silicon Fuzz","Octave Fuzz","Velcro Fuzz"};
...
void PedalPage::loadSlot()
{
    ...
    enabledButton.setToggleState(c.enabled.load(),juce::dontSendNotification); modelSelector.setSelectedId(juce::jlimit(0,8,c.model.load())+1,juce::dontSendNotification);
...
{
    const int m=juce::jlimit(0,8,modelSelector.getSelectedId()-1); blend.setEnabled(m==3); blend.setName(m==3?"CLEAN BLEND":"BLEND (N/A)");
    switch((PedalType)m)
    {
