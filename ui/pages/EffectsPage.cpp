...
EffectsPage::EffectsPage(guitardsp::hq::HQEffectsRack& r) : rack(r)
{
    ...
    for (auto* s : {&modRate,&modDepth,&modMix,&modFeedback,&modManual,&modShape,&delayTime,&delayFeedback,&delayMix,&delayLowCut,&delayHighCut,&delayDrive,&delayWow,&delayFlutter,&delayAge,&reverbSize,&reverbDecay,&reverbDamping,&reverbPreDelay,&reverbMix,&reverbMod,&reverbDrip})
        addAndMakeVisible(*s);

    modType.onChange=[this]{pushMod();updateModeHints();};
    for(auto* s:{&modRate,&modDepth,&modMix,&modFeedback,&modManual,&modShape}) s->onValueChange=[this]{pushMod();};

    delayEnabled.onClick=[this]{rack.setDelayEnabled(delayEnabled.getToggleState());}; delayType.onChange=[this]{pushDelay();updateModeHints();};
    for(auto* s:{&delayTime,&delayFeedback,&delayMix,&delayLowCut,&delayHighCut,&delayDrive,&delayWow,&delayFlutter,&delayAge}) s->onValueChange=[this]{pushDelay();};

    reverbEnabled.onClick=[this]{rack.setReverbEnabled(reverbEnabled.getToggleState());}; reverbType.onChange=[this]{pushReverb();updateModeHints();};
    for(auto* s:{&reverbSize,&reverbDecay,&reverbDamping,&reverbPreDelay,&reverbMix,&reverbMod,&reverbDrip}) s->onValueChange=[this]{pushReverb();};
...
void EffectsPage::pushMod()
{
    rack.setModulationMode((guitardsp::hq::HQEffectsRack::ModulationMode)juce::jlimit(0,5,modType.getSelectedId()-1));
    auto& c=rack.modulationControl(); c.rateHz.store((float)modRate.getValue()); c.depth.store((float)modDepth.getValue()); c.mix.store((float)modMix.getValue()); c.feedback.store((float)modFeedback.getValue()); c.manual.store((float)modManual.getValue()); c.shape.store((float)modShape.getValue());
}

void EffectsPage::pushDelay()
{
    auto& c=rack.delayControl(); c.flavor.store(juce::jlimit(0,2,delayType.getSelectedId()-1)); c.timeMs.store((float)delayTime.getValue()); c.feedback.store((float)delayFeedback.getValue()); c.mix.store((float)delayMix.getValue()); c.lowCutHz.store((float)delayLowCut.getValue()); c.highCutHz.store((float)delayHighCut.getValue()); c.drive.store((float)delayDrive.getValue()); c.wow.store((float)delayWow.getValue()); c.flutter.store((float)delayFlutter.getValue()); c.age.store((float)delayAge.getValue());
}

void EffectsPage::pushReverb()
{
    auto& c=rack.reverbControl(); c.flavor.store(juce::jlimit(0,3,reverbType.getSelectedId()-1)); c.size.store((float)reverbSize.getValue()); c.decay.store((float)reverbDecay.getValue()); c.damping.store((float)reverbDamping.getValue()); c.preDelayMs.store((float)reverbPreDelay.getValue()); c.mix.store((float)reverbMix.getValue()); c.mod.store((float)reverbMod.getValue()); c.drip.store((float)reverbDrip.getValue());
}
...
void EffectsPage::updateModeHints()
{
    ...
    const int d=juce::jlimit(0,2,delayType.getSelectedId()-1); delayHint.setText(d==0?"DIGITAL: clean repeats":d==1?"ANALOG: saturated/darker feedback; DRIVE matters":"TAPE: DRIVE + WOW + FLUTTER + AGE shape the repeats",juce::dontSendNotification);
    const int r=juce::jlimit(0,3,reverbType.getSelectedId()-1); reverbHint.setText(r==0?"ROOM: compact natural ambience":r==1?"PLATE: dense smooth reflections":r==2?"HALL: longer/open ambience":"SPRING: DRIP is the key character control",juce::dontSendNotification);
}
...
void EffectsPage::resized()
{
    ...
    auto modHead=mod.removeFromTop(30); modTitle.setBounds(modHead.removeFromLeft(120)); modType.setBounds(modHead.removeFromLeft(150)); modHint.setBounds(modHead);
    juce::Slider* ms[]={&modRate,&modDepth,&modMix,&modFeedback,&modManual,&modShape}; const char* mn[]={"RATE","DEPTH","MIX","FEEDBACK","MANUAL","SHAPE"}; int mw=mod.getWidth()/6; for(int i=0;i<6;++i){auto cell=mod.removeFromLeft(mw); ms[i]->setName(mn[i]); ms[i]->setBounds(cell.reduced(4));}

    auto dHead=del.removeFromTop(30); delayTitle.setBounds(dHead.removeFromLeft(85)); delayEnabled.setBounds(dHead.removeFromLeft(110)); delayType.setBounds(dHead.removeFromLeft(130)); delayHint.setBounds(dHead);
    juce::Slider* ds[]={&delayTime,&delayFeedback,&delayMix,&delayLowCut,&delayHighCut,&delayDrive,&delayWow,&delayFlutter,&delayAge}; int dw=del.getWidth()/9; for(auto* s:ds){s->setBounds(del.removeFromLeft(dw).reduced(2));}

    auto rHead=rev.removeFromTop(30); reverbTitle.setBounds(rHead.removeFromLeft(95)); reverbEnabled.setBounds(rHead.removeFromLeft(120)); reverbType.setBounds(rHead.removeFromLeft(130)); reverbHint.setBounds(rHead);
    juce::Slider* rs[]={&reverbSize,&reverbDecay,&reverbDamping,&reverbPreDelay,&reverbMix,&reverbMod,&reverbDrip}; int rw=rev.getWidth()/7; for(auto* s:rs){s->setBounds(rev.removeFromLeft(rw).reduced(3));}
}
