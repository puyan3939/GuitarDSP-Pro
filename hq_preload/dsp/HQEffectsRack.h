...
public:
    ...
    struct PedalSlotControl {
        ...
        // Bit mask: MAIN=1, CLEAN=2, SUB=4. Multiple destinations are allowed.
        std::atomic<int> routeMask{routeMain};
    };
    struct GateControl { std::atomic<float> thresholdDb{-55}, rangeDb{-60}, ratio{4}; std::atomic<float> attackMs{1}, holdMs{35}, releaseMs{180}, hysteresisDb{4}; std::atomic<float> sidechainHpHz{55}, sidechainLpHz{6500}; };
...
private:
    ...
    // Independent state per route is required for nonlinear/stateful pedals in parallel paths.
    ...
    std::array<ChorusHQ, stereoChannels> chorus; std::array<FlangerHQ, stereoChannels> flanger; std::array<PhaserHQ, stereoChannels> phaser; std::array<TremoloHQ, stereoChannels> tremolo; std::array<VibratoHQ, stereoChannels> vibrato;
    ModulationControl modControls; std::array<DelayHQ, stereoChannels> delayFx; std::array<ReverbHQ, stereoChannels> reverbFx; DelayControl delayControls; ReverbControl reverbControls; ParallelRigControl parallelControls;
    juce::AudioBuffer<float> pedalMonoWork; juce::AudioBuffer<float> gateCleanKey; int preparedMaxBlock = 0;
    std::atomic<int> dynamicsMode{(int)DynamicsMode::off}; std::atomic<int> modulationMode{(int)ModulationMode::off}; std::atomic<bool> delayEnabled{false}; std::atomic<bool> reverbEnabled{false};
};
}
